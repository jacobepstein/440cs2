...
shinyApp(
  ui = fluidPage(
    ...
    sidebarLayout(
      ...
      mainPanel = mainPanel(tabsetPanel(tabPanel(title = "Visualization", plotOutput("scatter")),   tabPanel(title = "Data",dataTableOutput(outputId = "table")
          ))))), 
    ...
    observeEvent({
     ...
     output$scatter = renderPlot({
       ...
       if(nrow(regionset)==0){
         ...
         ggplot(data = regionset, aes(x= x, y=y, na.rm = TRUE))+ geom_point(aes(colour = factor(regionset$Region), size=Population)) + ylab(input$outcome) + xlab(input$selector) + geom_smooth(method='lm',formula=y~x) + labs(color="Region") + ggtitle(paste(paste(input$outcome, " VS "), input$selector)) + theme_minimal()
       }
       })
